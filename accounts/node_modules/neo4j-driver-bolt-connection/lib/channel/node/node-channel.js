...
var TrustStrategy = {
    TRUST_CUSTOM_CA_SIGNED_CERTIFICATES: function (config, onSuccess, onFailure) {
        if (!config.trustedCertificates ||
            ...
            onFailure(neo4j_driver_core_1.newError('You are using TRUST_CUSTOM_CA_SIGNED_CERTIFICATES as the method ' +
        ...
        var socket = tls_1.default.connect(config.address.port(), config.address.resolvedHost(), tlsOpts, function () {
            if (!socket.authorized) {
                onFailure(neo4j_driver_core_1.newError('Server certificate is not trusted. If you trust the database you are connecting to, add' +
    ...
    TRUST_SYSTEM_CA_SIGNED_CERTIFICATES: function (config, onSuccess, onFailure) {
        ...
        var socket = tls_1.default.connect(config.address.port(), config.address.resolvedHost(), tlsOpts, function () {
            if (!socket.authorized) {
                onFailure(neo4j_driver_core_1.newError('Server certificate is not trusted. If you trust the database you are connecting to, use ' +
    ...
    TRUST_ALL_CERTIFICATES: function (config, onSuccess, onFailure) {
        ...
        var socket = tls_1.default.connect(config.address.port(), config.address.resolvedHost(), tlsOpts, function () {
            ...
            if (isEmptyObjectOrNull(certificate)) {
                onFailure(neo4j_driver_core_1.newError('Secure connection was successful but server did not return any valid ' +
...
function _connect(config, onSuccess, onFailure) {
    ...
    if (!isEncrypted(config)) {
        ...
        onFailure(neo4j_driver_core_1.newError('Unknown trust strategy: ' +
            config.trust +
            '. Please use either ' +
...
var NodeChannel = /** @class */ (function () {
    ...
    NodeChannel.prototype._handleConnectionError = function (err) {
        ...
        this._error = neo4j_driver_core_1.newError(msg, this._connectionErrorCode);
        if (this.onerror) {
    ...
    NodeChannel.prototype._handleConnectionTerminated = function () {
        this._open = false;
        this._error = neo4j_driver_core_1.newError('Connection was closed by server', this._connectionErrorCode);
        if (this.onerror) {
            this.onerror(this._error);
        }
    };
    ...
    NodeChannel.prototype._setupConnectionTimeout = function (config, socket) {
        ...
        if (timeout) {
            ...
            var timeoutListener_1 = function () {
                ...
                socket.setTimeout(0);
                socket.destroy(neo4j_driver_core_1.newError("Failed to establish connection in " + timeout + "ms", config.connectionErrorCode));
            };
    ...
    NodeChannel.prototype.setupReceiveTimeout = function (receiveTimeout) {
        ...
        this._conn.on('timeout', function () {
            _this._conn.destroy(neo4j_driver_core_1.newError("Connection lost. Server didn't respond in " + receiveTimeout + "ms", _this._connectionErrorCode));
        });
    ...
    NodeChannel.prototype.write = function (buffer) {
        ...
        if (this._pending !== null) {
            ...
            throw neo4j_driver_core_1.newError("Don't know how to write: " + buffer);
        }
    };
    /**
