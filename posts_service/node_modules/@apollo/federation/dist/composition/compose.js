...
function buildSchemaFromDefinitionsAndExtensions({ typeDefinitionsMap, typeExtensionsMap, directiveDefinitionsMap, directiveMetadata, serviceList, }) {
    ...
    const otherKnownDirectivesToInclude = directives_1.otherKnownDirectives.filter((directive) => directiveMetadata.hasUsages(directive.name));
    ...
    schema = new graphql_1.GraphQLSchema({
        ...schema.toConfig(),
        directives: [
            ...schema.getDirectives().filter((x) => !(0, directives_1.isFederationDirective)(x)),
        ],
    });
